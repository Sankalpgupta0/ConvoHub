...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (socketConnection) {
      socketConnection.emit('sidebar-conv', user._id)
    ...
    <div className='w-full h-full grid grid-cols-[48px,1fr] bg-white'>
      <div className='bg-slate-100 w-12 h-full rounded-tr-lg rounded-br-lg py-5 text-slate-600 flex flex-col justify-between'>
        <div>
          ...
          <div title='add friend' onClick={() => setOpenCreateGroup(true)} className='w-12 h-12 flex justify-center items-center cursor-pointer hover:bg-slate-200 rounded' >
            <MdGroupAdd size={20} />
          </div>
        </div>
      ...
      <div className='w-full'>
        ...
        <div className=' h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
          ...
          {
            groups && groups.length == 0 ? "" : <h1 className=' text-primary text-center text-xl font-semibold'>Groups</h1>
          }
          {
                ...
                <NavLink to={"/group/" + group?._id} key={group?._id} className='flex items-center gap-2 py-3 px-2 border border-transparent hover:border-primary rounded hover:bg-slate-100 cursor-pointer'>
                  ...
                  <div>
                    ...
                    <div className='text-slate-500 text-xs flex items-center gap-1'>
                      <div className='flex items-center gap-1'>
                        {
                          group?.lastMsg?.imageUrl && (
                            <div className='flex items-center gap-1'>
                              <span><FaImage /></span>
                              {!group?.lastMsg?.text && <span>Image</span>}
                            </div>
                          )
                        }
                        {
                          group?.lastMsg?.videoUrl && (
                            <div className='flex items-center gap-1'>
                              <span><FaVideo /></span>
                  ...
                  {/* {
                    Boolean(group?.unseenMsg) && (
                      <p className='text-xs w-6 h-6 flex justify-center items-center ml-auto p-1 bg-primary text-white font-semibold rounded-full'></p>
                    )
                  } */}
          ...
          {
            allUser.length == 0 ? "" : <h1 className=' text-primary text-center text-xl font-semibold'>Conversations</h1>
          }
          {
